...
const ENTITY_META: Record<string, { icon: typeof HardHat; restorePath?: string }> = {
  Agent: { icon: HardHat, restorePath: 'agents' },
  Client: { icon: Users, restorePath: 'clients' },
  Area: { icon: MapPinned, restorePath: 'areas' },
  Building: { icon: Building2, restorePath: 'buildings' },
  Admin: { icon: ShieldCheck },
};

// An entity type is not a state: the icon and the label already say which one
// it is, so colour stays reserved for what actually happened to the row.
const ENTITY_CHIP = 'text-neutral bg-neutral-wash border-neutral/20';
                ...
                {deletedItems.map(item => {
                  ...
                  return (
                    <div key={key} className="flex items-center gap-3 p-3 rounded-xl border border-border bg-muted/30 hover:bg-muted/60 transition-colors">
                      <div className={`h-10 w-10 rounded-lg border flex items-center justify-center shrink-0 ${ENTITY_CHIP}`}>
                        <Icon className="h-4 w-4" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <Badge variant="outline" className={ENTITY_CHIP}>{item.entityType}</Badge>
                      ...
                      <Button
                        ...
                        className="text-xs shrink-0 gap-1.5 border-success/30 text-success hover:bg-success-wash hover:text-success"
                      >
                  ...
                  return (
                    <div key={log.id} className="p-3 rounded-xl border border-border bg-muted/30">
                      <div className="flex items-start gap-3">
                        <div className={`h-9 w-9 rounded-lg border flex items-center justify-center shrink-0 ${ENTITY_CHIP}`}>
                          <Icon className="h-4 w-4" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            <Badge variant="outline" className={ENTITY_CHIP}>{log.entityType}</Badge>
                            {log.action === 'RESTORE' ? (
                              <Badge variant="outline" className="text-success bg-success-wash border-success/20 gap-1">
                                <Undo2 className="h-3 w-3" /> Restauré
                              </Badge>
                            ) : (
                              <Badge variant="outline" className="text-accent-foreground bg-accent border-primary/20 gap-1">
